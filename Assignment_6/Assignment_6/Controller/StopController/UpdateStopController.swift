...
class UpdateStopController: UIViewController {
    ...
    @IBAction func createStop(_ sender: CustomButton) {
        ...
        guard let stop = schedule.searchStop(name: stopname) else {
        ...
        let alertController = UIAlertController(title: "Success:", message: "Stop updated successfully", preferredStyle: .alert)
        let OKAction = UIAlertAction(title: "OK", style: .default, handler: nil)
        alertController.addAction(OKAction)
        self.present(alertController, animated: true, completion: nil)
        stopNameText.text = ""
