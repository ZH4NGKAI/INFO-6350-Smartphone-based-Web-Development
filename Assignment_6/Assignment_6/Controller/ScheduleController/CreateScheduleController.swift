...
class CreateScheduleController: UIViewController {
    ...
    @IBAction func createSchedule(_ sender: CustomButton) {
        ...
        train.add_Schedule(arrival: arrivalTime, departure: departureTime)
        let alertController = UIAlertController(title: "Success:", message: "Schedule added in Train \(trainID) successfully", preferredStyle: .alert)
        let OKAction = UIAlertAction(title: "OK", style: .default, handler: nil)
        alertController.addAction(OKAction)
        self.present(alertController, animated: true, completion: nil)
        trainIDText.text = ""
